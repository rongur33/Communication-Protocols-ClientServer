...
import bgu.spl.net.impl.tftp.TerminateThreads;
...
public class ClientTftpProtocol implements MessagingProtocol<byte[]> {
    ...
    private byte[] fileToData(String uploadFile) {
        String filePath = "." +File.separator+ uploadFile;
        try {
    ...
    public byte[] receiveData(byte[] msg){
            ...
            for(int i=0;i<data.length;i++){
                if(data[i]==0){
                    if(currName!=null&&begin==0){
                        ...
                        begin=i+1;
                        currName=null;
                    }else if(data[i]==0 &&currName==null) {
                ...
                } else if(data[i]!=0 && i==data.length-1){
                    currName=Arrays.copyOfRange(data, begin, i);
                }
            }
            short length= 4;
        ...
        if(currOpcode==Opcode.RRQ){
            ...
            if(separateArray.length<512){
                try (FileOutputStream fos = new FileOutputStream(Paths.get(directoryPath, fileToBeCreated).toString())) {
                    for(byte[] data:newFileData) {
                        fos.write(data);
                    }
                } catch (IOException e) {e.printStackTrace();}
                String srcFilePath = "src"+File.separator+fileToBeCreated; // Change this to the actual path of your file in the src directory
                ...
                System.out.println("RRQ "+fileToBeCreated+" completed");
            }
        }
        return ans;
    }
